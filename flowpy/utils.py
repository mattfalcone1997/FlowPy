...
def check_paths(path_to_folder, *folder_options):
    ...
    msg = "Directory(ies) %s cannot be found in path: %s" % (
        folder_options, path_to_folder)
...
def file_extract(path_to_folder, abs_path=True):
    ...
    file_names = [os.path.join(full_path, f)
                  for f in os.listdir(full_path) if f[:8] == 'DNS_peri']
    return file_names
