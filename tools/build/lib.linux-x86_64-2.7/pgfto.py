...
class PgfTo:
    def __init__(self,input_fn,preamble=None,latex_cmd='pdflatex',preview=False,doc_options=None,dependent_files=None):
        ...
        self._texGen.add_to_preamble(texCommand('tikzexternalize'))
    ...
    def to_eps(self,output_fn=None):
        ...
        t2 = NamedTemporaryFile(suffix='.eps',dir=self._tempdir.name)
        
        self._texSys.set_filename(t.name)
        
        out = self._texSys.run_latex(self._texGen,'-output-format=dvi','-shell-escape')
        ...
        run_dvips(t_prefix + '.dvi',t_prefix + '.eps')
        run_epstool(t_prefix + '.eps',t2.name)
        
        copy(t2.name,os.path.join(root,base_fn + '.eps'))
