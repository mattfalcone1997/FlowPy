...
class FlowFigure(mpl.figure.Figure):
...
mpl.projections.register_projection(FlowAxes3D)
...
def subplots(nrows=1, ncols=1, sharex=False, sharey=False, squeeze=True, subplot_kw=None, gridspec_kw=None,*args, **fig_kw):
    fig = plt.figure(FigureClass=FlowFigure,*args,**fig_kw)
    if subplot_kw is None:
        subplot_kw = {'projection':'FlowAxes'}
...
def _upgrade_fig(fig):
    if not isinstance(fig, FlowFigure):
        if isinstance(fig,mpl.figure.Figure):
            d = fig.__getstate__()
            fig = FlowFigure()
            fig.__setstate__(d)
        else:
            msg = f"fig needs to be an instance of {FlowFigure.__name__}"
            raise TypeError(msg)
...
def _upgrade_ax(fig,ax):
    fig = _upgrade_fig(fig)
    if not isinstance(ax,(FlowAxes, FlowAxes3D)):
        if isinstance(ax,mpl.axes.Axes):
            ...
            msg = f"ax needs to be an instance of {FlowAxes.__name__}"
            raise TypeError(msg)
    return ax
