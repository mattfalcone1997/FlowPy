...
from .plotting import (FlowFigure,
                       FlowAxes,
                       FlowAxes3D,
                       update_line_kw,
                       create_fig_ax_with_squeeze,
                       get_contour_func_params,
                       update_mesh_kw,
                       update_quiver_kw)
...
class _FlowStruct_base(datastruct):
    ...
    def check_shape(self) -> bool:
        ...
        if not all(s in coord_shapes for s in self.shape):
            msg = "The coordinate data is of different shape %s to the array data %s" % (
                coord_shapes, self.shape)
            raise ValueError(msg)
...
class FlowStructND(_FlowStruct_base):
    ...
    def to_hdf(self, filepath: str,
        ...
        if self._polar_plane is not None:
            hdf_obj.attrs['polar_plane'] = np.array(
                self._polar_plane, dtype=np.string_)
        if self._wall_normal_line is not None:
            hdf_obj.attrs['wall_normal_line'] = str(self._wall_normal_line)
        for k, v in self.attrs.items():
            hdf_obj.attrs[k] = v
        hdf_obj.attrs['data_layout'] = np.array(
            self._data_layout, dtype=np.string_)
    ...
    def _file_extract(self, filename, key=None):
        ...
        data_layout = list(x.decode('utf-8')
                           for x in hdf_obj.attrs['data_layout'])
        polar_plane = None
        wall_normal_line = None
        self.attrs = {}
        for key in hdf_obj.attrs:
            if key == 'polar_plane':
                polar_plane = list(x.decode('utf-8')
                                   for x in hdf_obj.attrs['polar_plane'])
    ...
    def plot_line(self, comp: str,
                  ...
                  ax: FlowAxes = None,
                  line_kw: dict = None,
                  **kwargs) -> Union[FlowFigure, FlowAxes]:
        """
    ...
    def plot_contour(self, comp: str,
                     ...
                     **kwargs) -> Union[FlowFigure, mpl.collections.PolyCollection]:
        """
        ...
        Returns
        -------
        Union[FlowFigure,mpl.collections.PolyCollection ]
        ...
        plot_func, contour_kw = get_contour_func_params(ax, contour_kw)
    ...
    def plot_vector(self, comps: str,
                    ...
                    ax: FlowAxes = None,
                    quiver_kw: dict = None,
                    **kwargs) -> Union[FlowFigure, mpl.quiver.Quiver]:
        """
        ...
        Returns
        -------
        Union[FlowFigure,mpl.quiver.Quiver]
            [description]
        """
        ...
        fig, ax = create_fig_ax_with_squeeze(fig, ax, **kwargs)
        quiver_kw = update_quiver_kw(quiver_kw)
        ...
        ax = ax.quiver(coord_1_mesh, coord_2_mesh, U.T, V.T,
                       angles='uv', scale_units='xy', scale=scale, **quiver_kw)
    ...
    def plot_isosurface(self, comp: str,
                        ...
                        ax: FlowAxes = None,
                        surf_kw: dict = None,
                        **kwargs) -> Union[FlowFigure, FlowAxes3D]:
        """
        ...
        Returns
        -------
        Union[FlowFigure,FlowAxes3D]
            [description]
        """
        self._check_dim(3)

        fig, ax = create_fig_ax_with_squeeze(
            fig, ax, projection='3d', **kwargs)
        surf_kw = update_mesh_kw(surf_kw)
        ...
        Z, Y, X = meshgrid(coords3, coords2, coords1)

        flow_array = self[time, comp]

        ax = ax.plot_isosurface(Z, X, Y, flow_array, Value, **surf_kw)

        coord_lims = [np.amax(Z) - np.amin(Z), np.amax(X) -
                      np.amin(X), np.amax(Y) - np.amin(Y)]
    ...
    def join(cls, new_axis, axis_vals, flowstructs, index=None, wall_normal=False):
        ...
        coord_nd[new_axis] = np.array(
            cls._estimate_grid_from_points(axis_vals))
...
class FlowStruct3D(FlowStructND):
    ...
    def _array_ini(self, array, index=None,  copy=False):
        ...
        super()._array_ini(array, index=index, data_layout=data_layout,
                           wall_normal_line=wall_normal_line, polar_plane=polar_plane, copy=copy)
    ...
    def _dict_ini(self, dict_data, copy=False):
        ...
        super()._dict_ini(dict_data, data_layout=data_layout,
                          wall_normal_line=wall_normal_line, polar_plane=polar_plane, copy=copy)
    ...
    def plot_isosurface(self, comp, Value, time=None, y_limit=None, x_split_pair=None, fig=None, ax=None, surf_kw=None, **kwargs):
        ...
        if x_split_pair is not None:
            flowstruct = flowstruct.slice[:, :,
                                          x_split_pair[0], x_split_pair[1]]
...
class _FlowStruct_slicer:
    ...
    def _get_new_coorddata(self, output_slice, output_slice_nd):
        ...
        new_Coord_ND_DF = coordstruct(
            new_nd_dict) if Coord_ND_DF is not None else None
        new_coorddata = AxisData(
            flow_struct.Domain, new_CoordDF, new_Coord_ND_DF)
